...
def estimate_position_value_from_row(conn: sqlite3.Connection, netuid: int, row: Dict[str, Any]) -> float:
    staked = safe_float(row.get("staked_tao"), None)
    if staked is not None and staked > 0:
        return round(staked, 6)
    basis = safe_float(row.get("amount_tao"), 0.0)
    if basis <= 0:
        return 0.0
    entry_mark = max(safe_float(row.get("entry_mark"), 1.0), 1e-9)
    current_mark = latest_mark_value_for_netuid(conn, netuid, fallback=entry_mark)
    return round(basis * (current_mark / entry_mark), 6)
...
def capture_chain_execution_snapshot(subtensor: Any, wallet: Optional[Any], hotkey_ss58: str, netuid: int) -> Dict[str, Any]:
    return {
        ...
        "stake_tao": get_wallet_stake_tao_on_subnet(subtensor, wallet, hotkey_ss58, netuid),
        "coldkey_ss58": _wallet_coldkey_ss58(wallet),
...
def update_position_state_on_entry(conn: sqlite3.Connection, decision: TradeDecision, result: Optional[Dict[str, Any]] = None) -> None:
    ...
    actual_staked = safe_float(reconciliation.get("actual_stake_delta_tao"), decision.amount_tao)
    actual_spend = max(0.0, actual_spend)
    actual_staked = max(0.0, actual_staked)
    prev_basis = safe_float(cur.get("amount_tao"), 0.0)
    prev_staked = safe_float(cur.get("staked_tao"), prev_basis)
    new_basis = prev_basis + actual_spend
    new_staked = prev_staked + actual_staked
    blended_entry_mark = (new_basis / max(new_staked, 1e-9)) if new_staked > 0 else max(safe_float(cur.get("entry_mark"), 1.0), 1e-9)
    cur["amount_tao"] = round(new_basis, 6)
    cur["staked_tao"] = round(new_staked, 6)
    cur["entry_mark"] = blended_entry_mark
    cur["last_mark"] = safe_float(reconciliation.get("after", {}).get("stake_tao"), latest_mark_value_for_netuid(conn, decision.netuid, fallback=blended_entry_mark))
    ...
    est_value = round(new_staked if new_staked > 0 else new_basis, 6)
    detail = {
        "reason": decision.reason,
        "requested_amount_tao": decision.amount_tao,
        "actual_wallet_debit_tao": actual_spend,
        "actual_stake_delta_tao": actual_staked,
    ...
    insert_pnl_journal(
        ...
        actual_spend,
        actual_spend,
        actual_staked if actual_staked > 0 else actual_spend,
        0.0,
        new_basis,
        est_value,
        json.dumps(detail, default=str),
...
def update_position_state_on_exit(conn: sqlite3.Connection, decision: TradeDecision, full_exit: bool, result: Optional[Dict[str, Any]] = None) -> None:
    ...
    prev_basis = safe_float(cur.get("amount_tao"), 0.0)
    prev_staked = safe_float(cur.get("staked_tao"), prev_basis)
    if prev_basis <= 0 and prev_staked <= 0:
        state.pop(key, None)
        save_position_state(conn, state)
        return
    actual_credit = safe_float(reconciliation.get("actual_wallet_credit_tao"), decision.amount_tao)
    actual_unstaked = safe_float(reconciliation.get("actual_stake_delta_tao"), decision.amount_tao)
    actual_credit = max(0.0, actual_credit)
    actual_unstaked = max(0.0, actual_unstaked)
    ratio = 1.0 if full_exit else clamp01(actual_unstaked / max(prev_staked, 1e-9))
    ...
    remaining_staked = 0.0 if full_exit else max(0.0, prev_staked - actual_unstaked)
    cur["realized_pnl_tao"] = round(safe_float(cur.get("realized_pnl_tao"), 0.0) + realized, 6)
    cur["last_mark"] = safe_float(reconciliation.get("after", {}).get("stake_tao"), latest_mark_value_for_netuid(conn, decision.netuid, fallback=max(safe_float(cur.get("entry_mark"), 1.0), 1.0)))
    cur["last_reconciled_at"] = now_ts()
    if remaining_basis <= 1e-9 or remaining_staked <= 1e-9:
        state.pop(key, None)
    else:
        cur["amount_tao"] = round(remaining_basis, 6)
        cur["staked_tao"] = round(remaining_staked, 6)
        state[key] = cur
    save_position_state(conn, state)
    ...
    detail = {
        ...
        "actual_stake_delta_tao": actual_unstaked,
        "fee_estimate_tao": safe_float(reconciliation.get("fee_estimate_tao"), 0.0),
    }
    insert_pnl_journal(
        conn,
        decision.netuid,
        decision.name,
        "EXIT_FULL" if remaining_basis <= 1e-9 or remaining_staked <= 1e-9 else "EXIT_PARTIAL",
        actual_unstaked if actual_unstaked > 0 else decision.amount_tao,
        basis_removed,
        actual_credit,
        realized,
        remaining_basis,
...
def current_positions_total_tao(conn: sqlite3.Connection) -> float:
    ...
    for row in state.values():
        total += safe_float(row.get("staked_tao"), safe_float(row.get("amount_tao"), 0.0))
    return total
def position_amount_for_netuid(conn: sqlite3.Connection, netuid: int) -> float:
    state = load_position_state(conn)
    row = state.get(str(netuid), {})
    return safe_float(row.get("staked_tao"), safe_float(row.get("amount_tao"), 0.0))
...
def traded_tao_today(conn: sqlite3.Connection) -> float:
    cutoff = now_ts() - 86400
    cur = conn.execute("SELECT COALESCE(SUM(amount_tao), 0) FROM trades WHERE ts >= ? AND status = 'ok'", (cutoff,))
    row = cur.fetchone()
    return safe_float(row[0], 0.0) if row else 0.0
...
def compute_entry_amount(
    ...
    remaining_cap = max(0.0, profile.max_position_tao - current_pos)
...
def execute_exit(
    ...
    try:
        unlock_wallet_if_possible(wallet, wallet_password)
        # Exit amounts are held alpha on the subnet, not TAO basis.
        # Use the subnet unit here so we sell the actual alpha held.
...
def format_exit_executed(result: Dict[str, Any], decision: Optional[TradeDecision] = None, sig: Optional[Signal] = None) -> str:
    tao_amt = safe_float(result.get("amount_tao"), safe_float(getattr(decision, "amount_tao", 0.0), 0.0))
    lines = ["✅ **Exit Executed**", ""]
    netuid = result.get("netuid", getattr(decision, "netuid", "?"))
    name = result.get("name") or (decision.name if decision else "")
    title = f"Sold SN{netuid} {name}".strip()
    lines.append(f"**{title}**")
    lines.append(f"TAO amount closed: `{tao_amt:.6f}`")
...
def print_positions(conn: sqlite3.Connection) -> None:
    ...
    for netuid, row in state.items():
        ...
        print(
            f"SN{netuid} {row.get('name', '')}: basis={basis:.6f} TAO | est_value={est_value:.6f} TAO | "
            f"uPnL={unrealized:+.6f} TAO ({roi:+.2f}%) | entry_mark={entry_mark:.6f} | current_mark={current_mark:.6f}",
            flush=True,
        )
...
def print_performance(conn: sqlite3.Connection) -> None:
    ...
    if not rows:
        ...
        for ts, netuid, name, event, amount_tao, cost_basis_tao, est_value_tao, realized_pnl_tao, rem_basis, rem_value in rows:
            ...
            print(
                f"{stamp} | {event} SN{netuid} {name} | amt={safe_float(amount_tao):.6f} | "
