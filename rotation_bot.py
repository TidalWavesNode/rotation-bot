...
def recent_exit_performance(conn: sqlite3.Connection, limit: int = 12) -> Dict[str, float]:
    rows = conn.execute(
        """
        SELECT realized_pnl_tao
        FROM pnl_journal
        WHERE event LIKE 'EXIT%'
        ORDER BY ts DESC, id DESC
        LIMIT ?
        """,
        (limit,),
    ).fetchall()
    values = [safe_float(r[0], 0.0) for r in rows]
    total = len(values)
    wins = sum(1 for v in values if v > 0)
    losses = sum(1 for v in values if v < 0)
    win_rate = (wins / total) if total else 0.5
    avg_realized = (_mean(values) if values else 0.0)
    return {
        "count": float(total),
        "wins": float(wins),
        "losses": float(losses),
        "win_rate": float(win_rate),
        "avg_realized": float(avg_realized),
    }


def market_regime_from_rows(raw_rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    if not raw_rows:
        return {
            "regime": "UNKNOWN",
            "avg_delta": 0.0,
            ...
            "positive_flow_ratio": 0.0,
            "low_impact_ratio": 0.0,
            "sample_size": 0,
        }
    ordered = sorted(
        raw_rows,
        key=lambda r: (safe_float(r.get("validator_delta"), 0.0), float(r.get("cluster", 0)), safe_float(r.get("tao_flow_proxy"), 0.0)),
        reverse=True,
    )
    focus = ordered[: max(8, min(24, len(ordered)))]
    avg_delta = _mean([safe_float(r.get("validator_delta"), 0.0) for r in focus])
    avg_breadth = _mean([float(r.get("cluster", 0.0)) for r in focus])
    positive_delta_ratio = _ratio_true([safe_float(r.get("validator_delta"), 0.0) > MIN_VALIDATOR_DELTA for r in focus])
    positive_flow_ratio = _ratio_true([
        safe_float(r.get("tao_flow_proxy"), 0.0) > 0 or safe_float(r.get("reserve_delta"), 0.0) > 0 for r in focus
    ])
    low_impact_ratio = _ratio_true([safe_float(r.get("impact500"), 99.0) <= 2.5 for r in focus])

    regime = "CHOPPY"
    if avg_delta >= 0.025 and avg_breadth >= 2.0 and positive_delta_ratio >= 0.35 and positive_flow_ratio >= 0.35:
        regime = "TRENDING"
    elif avg_delta >= 0.012 and avg_breadth >= 1.2 and positive_delta_ratio >= 0.20:
        regime = "NEUTRAL"

    return {
        "regime": regime,
        "avg_delta": float(avg_delta),
        "avg_breadth": float(avg_breadth),
        "positive_delta_ratio": float(positive_delta_ratio),
        "positive_flow_ratio": float(positive_flow_ratio),
        "low_impact_ratio": float(low_impact_ratio),
        "sample_size": len(focus),
    }


def market_regime_from_signals(signals: List[Signal]) -> Dict[str, Any]:
    rows = [
        {
            "validator_delta": sig.validator_delta,
            "cluster": sig.validator_cluster,
            "tao_flow_proxy": sig.tao_flow_proxy,
            "reserve_delta": sig.reserve_delta,
            "impact500": sig.impact500,
        }
    ...
    return market_regime_from_rows(rows)


def build_dynamic_profile(base_profile: Profile, conn: sqlite3.Connection, regime_stats: Dict[str, Any]) -> Profile:
    p = Profile(**asdict(base_profile))
    recent = recent_exit_performance(conn)
    regime = str(regime_stats.get("regime", "NEUTRAL") or "NEUTRAL").upper()

    if regime == "CHOPPY":
        p.entry_score_min = min(0.95, max(base_profile.entry_score_min, base_profile.entry_score_min + 0.05))
        p.min_entry_taostats_score = min(0.75, max(base_profile.min_entry_taostats_score, 0.60))
        p.max_entry_impact500 = min(base_profile.max_entry_impact500, 2.0)
        p.block_entry_if_impact1000_above = min(base_profile.block_entry_if_impact1000_above, 8.0)
        p.block_entry_if_reserve_below = max(base_profile.block_entry_if_reserve_below, 5000.0)
        p.trade_cooldown_seconds = max(base_profile.trade_cooldown_seconds, 10800)
        p.min_hold_time_seconds = max(base_profile.min_hold_time_seconds, 14400)
        p.max_new_entries_per_cycle = min(base_profile.max_new_entries_per_cycle, 1)
        p.max_exits_per_cycle = min(base_profile.max_exits_per_cycle, 1)
        p.max_trades_per_hour = min(base_profile.max_trades_per_hour, 2)
        p.percent_of_free_tao = min(base_profile.percent_of_free_tao, 3.0)
        p.max_position_pct = min(base_profile.max_position_pct, 15.0)
        p.max_position_tao = min(base_profile.max_position_tao, 0.5)
        p.partial_exit_pct = min(base_profile.partial_exit_pct, 25.0)
    elif regime == "TRENDING":
        p.entry_score_min = max(0.52, base_profile.entry_score_min - 0.04)
        p.min_entry_taostats_score = max(0.40, base_profile.min_entry_taostats_score - 0.05)
        p.max_entry_impact500 = min(max(base_profile.max_entry_impact500, 3.5), base_profile.max_entry_impact500 + 0.75)
        p.block_entry_if_impact1000_above = max(base_profile.block_entry_if_impact1000_above, 10.0)
        p.trade_cooldown_seconds = max(1800, min(base_profile.trade_cooldown_seconds, 3600))
        p.min_hold_time_seconds = max(5400, min(base_profile.min_hold_time_seconds, 7200))
        p.max_new_entries_per_cycle = max(base_profile.max_new_entries_per_cycle, 2)
        p.max_exits_per_cycle = max(base_profile.max_exits_per_cycle, 2)
        p.max_trades_per_hour = max(base_profile.max_trades_per_hour, 3)
        p.percent_of_free_tao = max(base_profile.percent_of_free_tao, 8.0)
        p.max_position_pct = max(base_profile.max_position_pct, 20.0)
        p.max_position_tao = max(base_profile.max_position_tao, 1.0)
    else:
        p.entry_score_min = max(base_profile.entry_score_min, 0.58)
        p.min_entry_taostats_score = max(base_profile.min_entry_taostats_score, 0.55)
        p.max_entry_impact500 = min(base_profile.max_entry_impact500, 2.5)
        p.block_entry_if_impact1000_above = min(base_profile.block_entry_if_impact1000_above, 10.0)
        p.block_entry_if_reserve_below = max(base_profile.block_entry_if_reserve_below, 3500.0)
        p.trade_cooldown_seconds = max(base_profile.trade_cooldown_seconds, 7200)
        p.min_hold_time_seconds = max(base_profile.min_hold_time_seconds, 10800)
        p.max_new_entries_per_cycle = min(base_profile.max_new_entries_per_cycle, 1)
        p.max_exits_per_cycle = min(base_profile.max_exits_per_cycle, 1)
        p.max_trades_per_hour = min(base_profile.max_trades_per_hour, 2)
        p.percent_of_free_tao = min(base_profile.percent_of_free_tao, 5.0)
        p.max_position_pct = min(base_profile.max_position_pct, 18.0)
        p.max_position_tao = min(base_profile.max_position_tao, 0.75)
        p.partial_exit_pct = min(base_profile.partial_exit_pct, 35.0)

    if recent["count"] >= 4:
        if recent["win_rate"] < 0.35 or recent["avg_realized"] < 0:
            p.entry_score_min = min(0.97, p.entry_score_min + 0.03)
            p.max_entry_impact500 = max(1.25, p.max_entry_impact500 - 0.4)
            p.min_entry_taostats_score = min(0.8, p.min_entry_taostats_score + 0.03)
            p.trade_cooldown_seconds = max(p.trade_cooldown_seconds, base_profile.trade_cooldown_seconds + 1800)
            p.max_new_entries_per_cycle = 1
            p.percent_of_free_tao = min(p.percent_of_free_tao, max(2.5, base_profile.percent_of_free_tao * 0.6))
        elif recent["win_rate"] > 0.60 and recent["avg_realized"] > 0:
            p.entry_score_min = max(0.50, p.entry_score_min - 0.02)
            p.max_entry_impact500 = min(base_profile.max_entry_impact500 + 1.0, p.max_entry_impact500 + 0.25)
            p.min_entry_taostats_score = max(0.35, p.min_entry_taostats_score - 0.02)
            p.max_new_entries_per_cycle = min(max(base_profile.max_new_entries_per_cycle, 2), p.max_new_entries_per_cycle + 1)
            p.percent_of_free_tao = min(max(base_profile.percent_of_free_tao, 8.0), p.percent_of_free_tao + 1.0)

    p.name = f"{base_profile.name} / {regime.title()}"
    return p
...
def build_signals(
    ...
    regime_stats = market_regime_from_rows(raw_rows)
    effective_profile = build_dynamic_profile(profile, conn, regime_stats)
    set_meta(conn, "dynamic_regime", regime_stats)
    set_meta(conn, "dynamic_profile", asdict(effective_profile))
    ...
    enrich_signals_with_rotation_metrics(signals, feature_history, watch_state, effective_profile)
    ...
    current_target = [n for n in derive_target_portfolio(signals, prev_target, effective_profile) if is_tradeable_netuid(n)]
    entries, holds, exits, watch = annotate_portfolio_actions(signals, prev_target, current_target, effective_profile)
...
def main() -> None:
    ...
    while True:
        ...
        try:
            ...
            effective_profile_raw = get_meta(conn, "dynamic_profile", {})
            try:
                effective_profile = Profile(**effective_profile_raw) if effective_profile_raw else profile
            except Exception:
                effective_profile = profile
            regime_stats = get_meta(conn, "dynamic_regime", {})
            ...
            if isinstance(regime_stats, dict):
                print(
                    f"[dynamic] regime={regime_stats.get('regime','UNKNOWN')} | "
                    f"entry_min={effective_profile.entry_score_min:.3f} | "
                    f"impact500_max={effective_profile.max_entry_impact500:.2f} | "
                    f"taostats_min={effective_profile.min_entry_taostats_score:.2f} | "
                    f"cooldown={effective_profile.trade_cooldown_seconds}s | "
                    f"hold={effective_profile.min_hold_time_seconds}s | "
                    f"size={effective_profile.percent_of_free_tao:.1f}%",
                    flush=True,
                )
            ...
            status_payload = {
                ...
                "dynamic_regime": regime_stats.get("regime", "UNKNOWN") if isinstance(regime_stats, dict) else "UNKNOWN",
                "dynamic_entry_score_min": effective_profile.entry_score_min if effective_profile else profile.entry_score_min,
                "dynamic_max_entry_impact500": effective_profile.max_entry_impact500 if effective_profile else profile.max_entry_impact500,
            }
            ...
            entry_decisions, exit_decisions = make_trade_decisions(conn, effective_profile, wallet_free_tao, entries, exits)
            ...
            if changed and profile.send_discord_updates:
                ...
                if not was_alerted_recently(conn, summary_key, 1800):
                    post_discord(
                        secrets.discord_webhook_url,
                        build_portfolio_update(effective_profile, current_target, signals, entries, exits, wallet_free_tao, conn),
                    )
