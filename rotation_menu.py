...
def reset_trading_state(conn: sqlite3.Connection) -> None:
    ...
    if was_running:
        print('Stopping rotation-bot.service...')
        result = service_cmd('stop')
        if result.returncode != 0:
            print('Failed to stop rotation-bot.service')
    ...
    try:
        ...
        reset_conn = sqlite3.connect(DB_PATH, timeout=10)
        try:
            reset_conn.execute('PRAGMA busy_timeout = 10000')
            reset_conn.execute('BEGIN IMMEDIATE')
            ...
            reset_conn.executemany('DELETE FROM meta WHERE key = ?', [(k,) for k in keys_to_remove])
            reset_conn.commit()
        finally:
            reset_conn.close()
        ...
        if 'locked' in str(exc).lower():
            print('Database is still locked. Make sure no other process is using rotation_bot.db.')
        time.sleep(2)
    finally:
        if was_running:
            print('Starting rotation-bot.service...')
            result = service_cmd('start')
            if result.returncode != 0:
                print('Reset completed, but failed to restart rotation-bot.service')
                if result.stderr.strip():
                    print(result.stderr.strip())
            else:
                print('rotation-bot.service restarted.')
        time.sleep(1)
...
def edit_saved_settings(conn: sqlite3.Connection) -> None:
    ...
    while True:
        ...
        if choice == '0':
            return
        if choice.lower() == 's':
            set_meta(conn, 'profile', profile)
            set_meta(conn, 'secrets', secrets)
            print('Saved.')
            time.sleep(1)
            return
        if choice.lower() == 'r':
