...
def dashboard(conn: sqlite3.Connection) -> None:
    ...
    open_basis = sum(float((row or {}).get('tao_basis', (row or {}).get('amount_tao', 0)) or 0) for row in position_state.values())
    open_staked = sum(float((row or {}).get('alpha_held', (row or {}).get('staked_tao', 0)) or 0) for row in position_state.values())
    ...
    for netuid, row in position_state.items():
        entry_mark = float((row or {}).get('entry_mark', 0) or 0)
        current_mark = float(((mark.get(str(netuid), {}) or {}).get('mark', entry_mark if entry_mark > 0 else 1)) or (entry_mark if entry_mark > 0 else 1))
        alpha_held = float((row or {}).get('alpha_held', (row or {}).get('staked_tao', 0)) or 0)
        open_value += alpha_held * current_mark
    print(f'Open est value TAO:{open_value:>11.6f}')
    print(f'Total alpha held:   {open_staked:.6f}')
...
def view_portfolio(conn: sqlite3.Connection) -> None:
    ...
    for netuid, row in state.items():
        basis = float((row or {}).get('tao_basis', (row or {}).get('amount_tao', 0)) or 0)
        alpha_held = float((row or {}).get('alpha_held', (row or {}).get('staked_tao', 0)) or 0)
        entry_mark = float((row or {}).get('entry_mark', 0) or 0)
        current_mark = float(((mark.get(str(netuid), {}) or {}).get('mark', entry_mark if entry_mark > 0 else 1)) or (entry_mark if entry_mark > 0 else 1))
        est_value = alpha_held * current_mark if alpha_held > 0 else 0.0
        upnl = est_value - basis
        print(f"SN{netuid} {(row or {}).get('name', '')}")
        print(f"  basis={basis:.6f} TAO | alpha_held={alpha_held:.6f} | current_mark={current_mark:.6f} TAO/alpha | est_value={est_value:.6f} TAO | est_uPnL={upnl:+.6f} TAO")
...
def view_trade_journal(conn: sqlite3.Connection) -> None:
    ...
    for ts, side, netuid, name, amount_tao, status, detail in rows:
        ...
        try:
            ...
            payload = {}
        label = 'tao' if str(side).upper() == 'ENTRY' else 'alpha'
        amount_display = float(payload.get('amount_alpha', amount_tao) if str(side).upper() == 'EXIT' else amount_tao)
        print(f"{fmt_ts(ts)} | {side:<5} SN{netuid:<3} {name:<24} {label}={amount_display:.6f} status={status}")
        if detail:
...
def pnl_summary(conn: sqlite3.Connection) -> None:
    ...
    total_alpha = 0.0
    for netuid, row in position_state.items():
        amt = float((row or {}).get('tao_basis', (row or {}).get('amount_tao', 0)) or 0)
        alpha_held = float((row or {}).get('alpha_held', (row or {}).get('staked_tao', 0)) or 0)
        entry_mark = float((row or {}).get('entry_mark', 0) or 0)
        current_mark = float(((mark.get(str(netuid), {}) or {}).get('mark', entry_mark if entry_mark > 0 else 1)) or (entry_mark if entry_mark > 0 else 1))
        basis += amt
        est_value += alpha_held * current_mark
        total_alpha += alpha_held
    ...
    print(f'Total alpha held:     {total_alpha:.6f}')
...
def reconcile_positions_now(conn: sqlite3.Connection) -> None:
    clear()
    print('=' * 72)
    print('Reconcile Positions From Chain')
    print('=' * 72)
    print('This refreshes tracked alpha holdings from on-chain stake data using your saved wallet and validator.')
    confirm = input('Type RECONCILE to continue: ').strip()
    ...
    try:
        result = subprocess.run(
            [sys.executable, str(Path(__file__).with_name('rotation_bot.py')), '--reconcile-only'],
            capture_output=True,
            text=True,
            check=False,
        )
        ...
        start_service_if_needed(was_running)
...
def main() -> None:
    while True:
        ...
        print('13. Reconcile positions from chain')
        print('0. Exit')
