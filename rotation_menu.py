...
def service_cmd(cmd: str) -> None:
    subprocess.run(['sudo', 'systemctl', cmd, SERVICE], check=False)
...
def reset_trading_state(conn: sqlite3.Connection) -> None:
    ...
    conn.execute('DELETE FROM pnl_journal')
    conn.execute('DELETE FROM execution_reconciliations')
    conn.execute('DELETE FROM alerts')
    conn.execute('DELETE FROM history')
    conn.execute('DELETE FROM validator_snapshots')
    ...
    conn.executemany('DELETE FROM meta WHERE key = ?', [(k,) for k in keys_to_remove])
    conn.commit()
