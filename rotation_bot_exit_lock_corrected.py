...
def is_exit_locked(conn: sqlite3.Connection, netuid: int, grace_seconds: int = 30) -> bool:
    state = load_position_state(conn)
    row = state.get(str(netuid), {})
    if not row:
        return False
    if bool(row.get("exit_in_progress")):
        return True
    last_attempt = safe_int(row.get("last_exit_attempt_ts"), 0)
    return last_attempt > 0 and (now_ts() - last_attempt) < grace_seconds

def set_exit_lock(conn: sqlite3.Connection, netuid: int, locked: bool, ts: Optional[int] = None) -> None:
    state = load_position_state(conn)
    key = str(netuid)
    ...
    if ts is None:
        ts = now_ts()
    row["last_exit_attempt_ts"] = int(ts)
    row["last_reconciled_at"] = int(row.get("last_reconciled_at", ts))
    state[key] = row
    save_position_state(conn, state)
...
def make_trade_decisions(
    ...
    seen_exit_netuids: set[int] = set()
    real_exits = [sig for sig in exits if has_open_position(conn, sig.netuid)]
    for sig in real_exits:
        if sig.netuid in seen_exit_netuids:
            continue
        seen_exit_netuids.add(sig.netuid)
        if len(exit_decisions) >= profile.max_exits_per_cycle:
            break
        if is_exit_locked(conn, sig.netuid):
            continue
...
def maybe_execute_trades(
    ...
    if exit_decisions and wallet is not None:
        dedup_netuids: List[int] = []
        seen_exit_netuids: set[int] = set()
        deduped_exit_decisions: List[TradeDecision] = []
        for d in exit_decisions:
            if d.netuid in seen_exit_netuids:
                continue
            seen_exit_netuids.add(d.netuid)
            dedup_netuids.append(d.netuid)
            deduped_exit_decisions.append(d)
        exit_decisions = deduped_exit_decisions
        reconcile_position_state_from_chain(conn, subtensor, wallet, profile, dedup_netuids)
        refreshed_exit_decisions: List[TradeDecision] = []
        for d in exit_decisions:
            if is_exit_locked(conn, d.netuid):
                continue
    ...
    for d in exit_decisions:
        ...
        if profile.execution_mode == "confirm" and not session_flags["auto_confirm"]:
            ...
            if action == "always":
                session_flags["auto_confirm"] = True
        set_exit_lock(conn, d.netuid, True)
        try:
            ...
            set_exit_lock(conn, d.netuid, False)
    if started_txn and conn.in_transaction:
        conn.commit()
    return entry_results, exit_results, session_flags
# ============================================================
