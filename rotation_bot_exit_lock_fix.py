...
def position_is_exit_locked(conn: sqlite3.Connection, netuid: int) -> bool:
    state = load_position_state(conn)
    row = state.get(str(netuid), {})
    return bool(row.get("exit_in_progress", False))


def set_position_exit_lock(conn: sqlite3.Connection, netuid: int, locked: bool, reason: str = "") -> None:
    state = load_position_state(conn)
    key = str(netuid)
    row = dict(state.get(key, {}) or {})
    if not row and not locked:
        return
    row["exit_in_progress"] = bool(locked)
    row["exit_lock_reason"] = reason if locked else ""
    row["exit_lock_ts"] = now_ts() if locked else 0
    state[key] = row
    save_position_state(conn, state)


def clear_position_exit_lock(conn: sqlite3.Connection, netuid: int) -> None:
    set_position_exit_lock(conn, netuid, False, "")
...
def reconcile_position_state_from_chain(
    ...
    for netuid in target_netuids:
        ...
        cur["exit_in_progress"] = False
        cur["exit_lock_reason"] = ""
        cur["exit_lock_ts"] = 0
...
def current_positions_total_tao(conn: sqlite3.Connection) -> float:
    ...
    for row in state.values():
        total += safe_float(row.get("alpha_held"), safe_float(row.get("staked_tao"), 0.0))
    return total
...
def make_trade_decisions(
    ...
    unique_exit_sigs: Dict[int, Signal] = {}
    for sig in exits:
        if sig.netuid not in unique_exit_sigs:
            unique_exit_sigs[sig.netuid] = sig
    real_exits = [sig for sig in unique_exit_sigs.values() if has_open_position(conn, sig.netuid)]
    for sig in real_exits[: profile.max_exits_per_cycle]:
        if position_is_exit_locked(conn, sig.netuid):
            continue
...
def execute_exit(
    ...
    try:
        ...
        validator_hotkey = get_validator_hotkey(profile)
        before = capture_chain_execution_snapshot(subtensor, wallet, validator_hotkey, decision.netuid)
        current_alpha = max(0.0, safe_float(before.get("alpha_held"), safe_float(before.get("stake_tao"), 0.0)))
        requested_alpha = max(0.0, float(decision.amount_tao))
        amount_alpha = requested_alpha
        if current_alpha > 0:
            if requested_alpha >= current_alpha * 0.999:
                amount_alpha = current_alpha
            else:
                amount_alpha = min(requested_alpha, current_alpha)
        amount = bt.Balance.from_tao(amount_alpha).set_unit(decision.netuid)
        result = _call_subtensor_method(
            subtensor,
        ...
        reconciliation = reconcile_execution_with_chain(
            subtensor, wallet, validator_hotkey, decision.netuid,
            side="EXIT", requested_amount_tao=amount_alpha,
            before=before, chain_success=chain_success,
        )
        return {
            "ok": chain_success,
...
def maybe_execute_trades(
    ...
    for d in exit_decisions:
        if position_is_exit_locked(conn, d.netuid):
            exit_results.append({"ok": False, "side": "EXIT", "netuid": d.netuid, "error": "exit already in progress"})
            continue
        ...
        if profile.execution_mode == "confirm" and not session_flags["auto_confirm"]:
            ...
            if action == "always":
                session_flags["auto_confirm"] = True
        set_position_exit_lock(conn, d.netuid, True, "submitted")
        ...
        if result.get("ok") and not result.get("dry_run"):
            ...
            reconcile_position_state_from_chain(conn, subtensor, wallet, profile, [d.netuid])
            post_alpha = position_amount_for_netuid(conn, d.netuid)
            actual_alpha_sold = safe_float(result.get("reconciliation", {}).get("actual_stake_delta_tao"), d.amount_tao)
            full_exit = post_alpha <= 1e-9 or actual_alpha_sold >= (post_alpha + actual_alpha_sold - 1e-9)
            update_position_state_on_exit(conn, d, full_exit=full_exit, result=result)
            clear_position_exit_lock(conn, d.netuid)
        else:
            clear_position_exit_lock(conn, d.netuid)
    if started_txn and conn.in_transaction:
        conn.commit()
...
def print_positions(conn: sqlite3.Connection) -> None:
    ...
    for netuid, row in state.items():
        basis = safe_float(row.get("amount_tao"), safe_float(row.get("tao_basis"), 0.0))
